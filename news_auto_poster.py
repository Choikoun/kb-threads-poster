...
CATEGORIES = {
    'business': {
        ...
        'format_variants': ['반전형', '사례형', '담백형', '번호형', '스토리형'],
        'hashtags': '#법인 #증여',
        'topic_tag': '법인'
    },
    'economy': {
        ...
        'angle': '경제·시장 이슈를 다루되, 사업주·법인 대표·자산가의 사업 자금·자산 운용과 자연스럽게 연결될 때만 그 관점을 살려라. 안 맞으면 억지로 갖다 붙이지 말고 이슈 자체로 다뤄도 된다.',
        'format_variants': ['반전형', '사례형', '담백형', '스토리형'],
        'hashtags': '#주식 #경제',
        'topic_tag': '경제'
    },
    'insurance': {
        ...
        'format_variants': ['반전형', '사례형', '감정인용형', '번호형', '스토리형'],
        'hashtags': '#증여 #상속',
        'topic_tag': '보험'
    },
    'policy': {
        ...
        'keywords': ['시행령', '개정', '정책', '법안', '국세청', '금융위', '기재부', '세법', '고시',
                     ...
                     '대법원', '헌재', '표결', '판결', '공정위',
                     '지원금', '환급', '보조금', '감면', '바우처'],
        'angle': '정부 정책·시행령·세법 개정을 다루되, 사업주·법인 대표·자산가 지갑에 실제로 영향이 있을 때만 그 관점을 살려라. 안 맞으면 억지로 갖다 붙이지 말고 정책 자체로 다뤄도 된다.',
...
[포인트 공식]·[메인 포스트 구조]는 이 경우 생략. 댓글 구조는 동일하게 유지.''',
    ...
    'government': {
        ...
        'angle': '국무회의·대통령 현안·정부 정책토론을 다루되, 사업주·법인 대표·자산가의 사업·자산에 실제로 영향이 있을 때만 그 관점을 살려라. 정치 얘기로 다루지는 말되, 억지 연결 없이도 그 자체로 임팩트 있으면 사업 영향 없이 다뤄도 된다.\n[주의 - 실제 반감 사례(2026-07-23)] 공무원 임금 인상 뉴스를 "결국 법인 인건비 부담으로 직결"이라고 억지로 엮었다가 독자들에게 "억지 연결"이라며 대거 비판받고 계정 정체성까지 조롱당함. 임금·처우·노동조건처럼 여론이 민감하게 갈리는 주제는 특히 신중하게 — 사실을 유리한 쪽으로만 골라 쓰지 말고(예: "인상률 3년 최고"만 강조하고 물가상승률·최저임금 대비 여전히 낮다는 맥락은 빼는 식 금지), 사업 영향 연결이 자연스럽지 않으면 이 뉴스를 선택하지 말고 다른 후보를 골라라.',
        'format_branch': '''선택한 뉴스가 그날 가장 화제인 정치 발언·논란(대통령/여야 발언, 헌법기관 충돌 등)이고 그 자체로 임팩트가 크면, 초단문 인용형으로 작성:
- 1~2줄: 발언/사실을 그대로 인용하거나 요약
↵빈줄
- 1줄: "대부분은 ~로 보지만, 진짜는 ~다" 식 재해석 한 줄 — 사업·자산에 자연스럽게 연결될 때만 그렇게 하고, 안 맞으면 이슈 자체의 재해석으로 남겨도 된다. 순수 정치 발언이라 어떤 식으로도 흥미로운 재해석이 안 나오면 이 뉴스를 선택하지 말고 다른 후보를 골라.
...
↵빈줄
2. 통념과 실제의 간극을 드러내는 충격 사실 — 각 문장 단독 줄
↵빈줄
3. 마무리 한 줄 — 아포리즘("아는 사람만 아는 얘기야"류) / 예측 / 남 얘기 아님 환기 중 매번 다르게. "~신호야", "~구조야" 금지.
...
↵빈줄
3. 마무리는 사례 주인공에게 돌아가서 끝낸다 — 그 사람의 결말·후회·한마디로. 일반론 마무리("결국 ~가 중요해")로 빠지지 말 것.
...
미리 알았으면 구조를 달리 짰을 텐데.''',
    ...
    '스토리형': '''시간 순서가 있는 미니 서사 5~7줄. 등장인물 하나(사업주/부부/상속인 등)가 결정을 내리고 → 시간이 흐르고 → 예상 못 한 결과를 맞는 흐름.
- 훅은 결말의 한 조각을 먼저 보여주고 시작해도 좋다 ("3년 뒤, 그 결정이 세금 고지서로 돌아왔어.")
- 숫자·사실은 서사 안에 자연스럽게 심는다. 별도 포인트 줄로 빼지 않는다.
- 마무리는 서사의 마지막 장면으로 끝낸다. 교훈 정리 문장("결국 ~해야 해")을 덧붙이지 않는다 — 장면이 교훈을 대신하게.''',

    '한줄형': '''1~3줄로 끝내는 초단문. 그날 이슈에 대한 관찰 하나 또는 촌철살인 한마디.
- 설명하지 않는다. 다 아는 이슈라는 전제로 툭 던진다.
- 숫자 하나 + 시선 하나면 충분 ("보유세 반대 50%. 정책이 아니라 민심이 먼저 움직였네.")
- 해시태그 제외 3줄 초과 금지. 댓글에서 살을 붙이는 건 가능.''',
...
↵빈줄
2. 번호 매긴 항목 3~5개 — 각 항목 "번호. 완결된 문장"으로, 항목당 1~2줄. 명사구를 대시(—)로 이어붙인 보고서식 나열 금지("A — B 인정" 같은 형식 금지) — 실제로 말하듯 서술어까지 다 붙여 써라.
↵빈줄
3. 마무리 한 줄 — 화두 던지고 끝
...
1. 거래처 밥값은 한도 안에서는 전액 다 인정해줘.
2. 사무실 비품이나 소모품도 살 때 바로 비용 처리 돼.
3. 명절에 직원들 선물 준 것도 일정 금액까지는 복리후생비로 잡을 수 있어.

몰랐으면 그냥 세후 지출, 알면 전부 비용 처리야.''',
}

# 담백형·번호형·한줄형은 ①숫자충격 ②반전 포인트 공식이 안 맞으므로 제외
POINT_FORMULA = '''[포인트 재료 - 최소 하나는 반드시, 조합·순서는 자유]
① 숫자/팩트 충격: 실제 수치, 공식 발표, 통계로 독자 멈추게
② 반전: 통념과 실제의 간극 ("대부분은 ~라고 생각하지?" 정형구 금지 — 간극 자체를 다른 방식으로 드러내라: 두 숫자 대비, 사례의 의외의 결말, 질문 없이 바로 사실 충돌 등)
③ 이해관계 뒤집기: 이 뉴스로 조용히 웃는 쪽 / 우는 쪽이 누군지 짚기

매번 같은 재료·같은 순서로 조립하지 마라. 어떤 글은 숫자로만 밀어붙이고, 어떤 글은 반전 없이 이해관계만 파도 된다.
'''
...
반말. "인스타에도 카드뉴스로 정리해서 올려" 같은 식으로 담백하게 사실만 언급. 매번 같은 문구 반복하지 않는다.
"@계정명" 같은 핸들은 만들어내지 않는다.
[금지 - 플랫폼이 금융사기로 자동 분류함] "놓치다/놓칠/기회/혜택/지금 아니면" 같이 긴급성·손실회피를 자극하는 표현 절대 금지.
"팔로우해", "팔로우해두면 다음 글도 볼 수 있어"처럼 정보 전달로만 끝낸다.
'''
...
반말. "이런 얘기 계속 보고 싶으면 팔로우해놔" 같은 담백한 표현. 주제에 맞게 변형해도 좋음. 매번 같은 문구 반복하지 않는다.
...
{news_list}
{trend_block}
[뉴스 선택 우선순위 — 핵심 기준: "사람이 나오는가"]
역대 최고 성과 글은 전부 실존 인물의 구체적 상황이 있는 뉴스였다(이혼 재산분할 34K·요양원 거부 노인 30K·직원 58억 횡령 15K·유언장 대법 판결 8.6K·간병인 돌변 사건). 반대로 주체가 기관이고 사람이 안 나오는 집계·통계는 숫자가 아무리 커도 죽는다(실측: 그런 주제로만 채운 주간은 TOP1이 1,203회에 그침).

1순위: **사람이 주인공인 돈·가족 사건/판결** — 이혼 재산분할, 상속·유언 분쟁, 간병·부양 갈등, 횡령·배임, 동업 배신, 사기 피해처럼 "누가 어떤 상황에서 어떻게 됐다"가 있는 뉴스
2순위: 전 국민·전 사업주에게 직접 닥치는 확정된 제도 변화 (기준금리 변경, 세법 시행 등)
3순위: 글로벌 빅테크 리더·재벌 총수 등 빅네임의 화제성 동향 (사업적 의미로 재해석 가능할 때만)

**강등 — 기관 집계·통계 발표("잔액 X조 돌파", "공고 X만 건", "X% 증가·감소" 류)는 위 1~3순위 후보가 하나도 없을 때만 선택.** 숫자 충격이 있어 보여도 독자에게 "내 얘기"가 아니라 반응이 약하다.
...
- 사람·전개 없이 "검거·적발됐다"로 끝나는 한 줄짜리 사건 단신 (반대로 인물의 상황과 전개가 담긴 사건 기사는 1순위다 — 혼동 금지)
...
- [금지 - 플랫폼이 금융사기로 자동 분류함] 특정 종목명(삼성전자, SK하이닉스 등)·레버리지·빚투와 구체적 금액(예: "현금 3천만원")을 한 포스트에 함께 언급하지 않는다. 단순 제도 뉴스 전달이어도 이 조합만으로 스캠성 콘텐츠로 자동 분류되어 게시물이 삭제된 사례 있음(2026-07-16). 종목명은 빼고 "레버리지 투자", "빚투" 등 일반화된 표현으로, 금액도 "수천만원 단위" 식으로 뭉뚱그려 쓴다.
- 완전한 답 주지 말 것. 경각심·인사이트·잘못된 상식을 건드리되 "상황마다 달라", "그때그때 다르게 풀어야 해"처럼 열어두어 독자 스스로 "내 상황은 어떻게 되지?" 상담 욕구가 생기도록.
- [판단·개성 문장] 매 포스팅에 최소 한 문장은 사실 요약이 아니라 "내 판단"이 드러나야 한다. 단, 표현 형태를 매번 바꿔라 — 단정("이건 방향이 정해진 얘기야"), 예측("6개월 안에 이 얘기 다시 나온다"), 소수의견("다들 A라는데 난 반대로 봐"), 우선순위 지정("여기서 볼 건 딱 하나야"), 경험 섞기("이런 케이스 여럿 봤는데 결말이 비슷해") 등.
- [상투구 완전 금지 — 예외 없음] 다음은 실제 사용자가 "AI 티 나고 너무 반복된다"고 지적한 표현이다. **아예 쓰지 마라**: "내가 볼 땐"(문장 시작), "~신호야"/"~구조야"(마무리), "대부분은 ~라고 생각하지?"/"~라고 믿지?" 류 질문형 훅. 판단 문장·마무리는 매번 다른 어휘로 새로 만들어라 — 같은 골격을 두 번째 쓰는 순간 그게 상투구다.
- [구어체 필수 — 보고서체 금지] 실제 대화하듯 써라. "~적용 및 공제 배제", "~압박이 세져", "~로 직결되는 구조" 같은 문어체·보고서체 표현 금지. 예: "공제 배제" → "공제 못 받아", "고용 비용 부담과 수익성 변화로 직결되는" → "결국 사장님들 인건비 부담으로 넘어와". 번호형처럼 항목을 나열할 때도 명사구로 딱딱하게 끊지 말고 짧은 문장으로 풀어써라.
- [가독성 — 한 문단에 문장 하나] 마침표 두 개 이상이 한 줄/한 문단에 붙어있으면 안 된다. "A했대. B라잖아." 처럼 두 문장을 이어 쓰지 말고 반드시 줄을 나눠라.
...
- [시각적 앵커 최소 1개] 텍스트만 쭉 이어지면 스크롤이 안 멈춘다. 이모지(💡⚠️📌🔑 등 의미 있는 것) 또는 구분 기호(• → ✓ 같은 점·화살표류) 중 최소 1개는 훅이나 핵심 줄에 배치해라. 단, 전체 포스트 기준 2~3개 넘지 않게 — 장식 남발은 스팸처럼 보인다.
...
- ✅ 자극적: "대부분이 이걸 몰라", "이거 해당되는 사람 많을 거야", "은행 좋은 일만 시키는 꼴이야"
...
- 마지막 댓글: 아래 중 매번 다른 방식을 골라서 마무리. "당신은 A야, B야?" 식 양자택일형 질문을 매번 반복하지 마라 — 전체 포스팅의 30% 이하로만 사용.
  - 양자택일형 질문 (가끔만): 독자가 댓글 하나로 바로 답할 수 있는 두 선택지
  - 자기 판단 던지기: "나는 이게 진짜 문제라고 봐" 식으로 단정하고 끝내기
  - 되묻는 질문: 앞서 나온 숫자·사실을 독자 상황에 되비추는 질문 (단, "어떻게 준비하고 있어?" 같은 완전 개방형은 금지 — 구체적 기준으로 되물을 것)
  - 다음 이야기 예고: 다루지 않은 후속 궁금증을 짧게 던지고 끝내기
  - 개방형 질문("어떻게 준비하고 있어?")은 위 되묻는 질문 형태로도 금지. 항상 구체적 기준·선택지가 있어야 함.
  - 2~3줄, 전부 반말
{follow_cta_block}
JSON만 출력:
{{
