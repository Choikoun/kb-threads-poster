...
↵빈줄
3. 마무리 한 줄 — 아포리즘("아는 사람만 아는 얘기야"류) / 예측 / 남 얘기 아님 환기 중 매번 다르게. "~신호야", "~구조야"로 끝내는 건 과사용이라 피할 것.
...
② 반전: 통념과 실제의 간극 (단, "대부분은 ~라고 생각하지?" 정형구는 이미 과사용 — 간극 자체를 다른 방식으로 드러내라: 두 숫자 대비, 사례의 의외의 결말, 질문 없이 바로 사실 충돌 등)
...
- [과사용 상투구 금지] 다음 표현은 이미 거의 매 글에 반복돼 상투구가 됐다. 같은 뜻을 매번 다른 말로 써라: "내가 볼 땐"(문장 시작), "~진짜 신호야", "~구조야"(마무리), "대부분은 ~라고 생각하지?". 이 단어들 자체를 아예 쓰지 말라는 게 아니라, 훅·마무리의 기본 템플릿처럼 기대지 말라는 것.
